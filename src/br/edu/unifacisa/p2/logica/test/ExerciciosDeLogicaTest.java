...
class ExerciciosDeLogicaTest {
	...
	void testInverteLista() {
		...
		assertArrayEquals(new String[] { "c", "b", "a" },
				ExerciciosDeLogica.inverteLista(nomes).toArray(new String[nomes.size()]));
	}
	...
	void testSubstitui() {
		...
		assertEquals("Para Ana: Ana, espero você na reunião.",
	...
	void testValidaCPF() {
		/*
		 * Um CPF válido tem: - 9 números separados por . - 2 dígitos verificadores
		 */
